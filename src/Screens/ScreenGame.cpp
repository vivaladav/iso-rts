...
void ScreenGame::HandleTempleOutcome(unsigned int outcome, Player * p, Temple * temple)
{
    const std::vector<GameObject *> & objs = mGameMap->GetObjects();
    ...
    if(outcome >= Temple::FIRST_EXP_REW && outcome <= Temple::LAST_EXP_REW)
    {
        ...
        switch(outcome)
        {
            ...
            case Temple::EXP_REW_INC_ENERGY_PRODUCTION:
            {
                ...
                for(GameObject * o : objs)
                {
                    if(o->GetObjectType() == GameObject::TYPE_RES_GEN_ENERGY)
            ...
            case Temple::EXP_REW_INC_MATERIAL_PRODUCTION:
            {
                ...
                for(GameObject * o : objs)
                {
                    if(o->GetObjectType() == GameObject::TYPE_RES_GEN_MATERIAL)
                        static_cast<ResourceGenerator *>(o)->ScaleOutput(mult);
                }
            }
            break;
