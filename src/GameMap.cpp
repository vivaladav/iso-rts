...
void GameMap::AddControlPointsForObject(GameObject * obj)
{
    ...
    for(int r = rTL; r <= rBR; ++r)
    {
        ...
        for(int c = cTL; c <= cBR; ++c)
        {
            ...
            mControlMap[ind].AddPoints(faction, maxPoints);
        }
    }
    ...
    for(int i = 1; i < maxPoints; ++i)
    {
        ...
        if(rT >= 0)
        {
            ...
            {
                const int ind = ind0 + c;
                mControlMap[ind].AddPoints(faction, p);
            }
        }
...
void GameMap::PrintControlMap(PlayerFaction f)
{
    for(unsigned int r = 0; r < mRows; ++r)
    {
        ...
        {
            const int ind = ind0 + c;
            std::cout << mControlMap[ind].points[f];
        }
...
void GameMap::ControlCell::UpdateRes()
{
    int maxPoints = 0;
